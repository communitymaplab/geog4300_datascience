---
title: "Geog4/6300: Two sample tests"
output: html_notebook
editor_options: 
  chunk_output_type: console
---

###Two sample tests

Two sample t tests are relatively simple in R. Let's start by creating two samples that we know are (slightly) different.

```{r}
library(tidyverse)

sample1<-data.frame(sample1=rnorm(50, mean=51.7, sd=6))
sample2<-data.frame(sample2=rnorm(50, mean=53.4, sd=3))

sample<-bind_cols(sample1,sample2) %>% #Combine the two variables
  mutate(row=row_number())
```

Do they look different?
```{r}
sample_long<-sample %>% pivot_longer(c(sample1,sample2),names_to="sample",values_to="value")
ggplot(sample_long,aes(y=value,x=sample)) + geom_boxplot() 
ggplot(sample_long,aes(x=value, fill=sample)) + 
  geom_histogram() + 
  facet_wrap(~sample)
```

The "t.test" function allows us to test them. The var.equal parameter allows us to specify if variances are equal (TRUE) or unequal (FALSE). Notice the different degrees of freedom between the two tests.
```{r}
t.test(sample1, sample2, var.equal=TRUE) 
t.test(sample1, sample2, var.equal=FALSE) 
```

What if the variance is really unequal?
```{r}
sample2<-data.frame(sample2=rnorm(50, mean=53.4, sd=50))
t.test(sample1, sample2, var.equal=FALSE)
```

You can also specify 1 or 2 tailed tests
```{r}
sample1<-data.frame(sample1=rnorm(50, mean=51.4, sd=6))
sample2<-data.frame(sample2=rnorm(50, mean=53.8, sd=3))

t.test(sample1, sample2, alternative="two.sided")
t.test(sample1, sample2, alternative="greater")
t.test(sample1, sample2, alternative="less")
```

##Two sample test of proportions

Proportions can be tested using prop.test. Let's check to see if we can detect a weighted coin in a con flip. First, we'll create a long table showing 100 flips of each coin.
```{r}
prop1<-data.frame(coin1=rbinom(100, 1 , .5)) #Regular coin
prop2<-data.frame(coin2=rbinom(100, 1 ,.75)) #Altered coin

prop_sample<-bind_cols(prop1,prop2) #Combine the two variables
prop_samplelong<-prop_sample %>% pivot_longer(c(coin1,coin2),names_to="coin",values_to="result") #Reshape the dataset so all values are in one column
```

Then we'll use group_by and summarise to get the counts of each outcome (0=tails, 1=heads). We can use prop.test to check whether these outcomes were significantly different. We use group_by and summarise to count the number of heads and tails by coin, spread those results to separate columns (labeled 0 and 1), and then calculate the total of all coin flips--100 in each case. The prop.test function looks at the numbers of "successes" and the total number of trials.

```{r}
prop_sample.table<-prop_samplelong %>%
  group_by(coin,result) %>%
  summarise(count=n()) %>%
  pivot_wider(names_from=result,values_from=count) %>%
  mutate(total=`0`+`1`)
prop.test(prop_sample.table$`1`,prop_sample.table$total) #Run prop.test on the table
```

You could also create this table yourself. Let's say coin 1 had 56 heads in 100 coin flips while coin 2 had 44 heads in 100 coin flips. Is there a difference? We can create one vector with the count of "successess" (in this case, the coin flip results in heads) and a second with the number of trials (100 flips in both). `prop.test` can then test for the difference.

```{r}
heads<-c(56, 44)
flips<-c(100, 100)

prop.test(heads,flips)
```


###Paired t-tests

Paired two sample tests also use t.test. Just add the parameter "paired=TRUE." Here, we look at readings from stream monitoring stations in 1990 and 2010.

```{r}
stream.1990<-c(334, 231, 261, 215, 209, 336, 393, 141, 185, 242, 122, 195, 421, 226)
stream.2010<-c(316, 217, 226, 176, 215, 327, 335, 155, 128, 244, 129, 175, 379, 196)
t.test(stream.1990, stream.2010, paired=TRUE) 
```

How does this compare to a regular t.test?
```{r}
t.test(stream.1990, stream.2010)
```


##You try it!

1) Let's compare liters of coffee sold on campus per 100 students each day on Mondays and Wednesdays at seven coffee stands. 

```{r}
mon<-c(12.4, 13.9, 14.6, 11.3, 18.1, 15.2, 13.9)
wed<-c(10.7, 11.1, 16.2, 12.5, 10.8, 16.7, 12.7)
```

Assume the data is normal. Run the appropriate t.test and then interpret the results. State which test you used, describe your results, and interpret what they tell you about a difference between these two days.

```{r}
#Code here
```

2) Motorists in two cities were surveyed about whether or not they carpooled when driving to work. In the first city, 398 motorists out of 985 answered yes. In the second, 445 of 912 did. Is there a difference in rates between these cities? State which test you used, describe your results, and interpret what they tell you about a difference between these cities.

```{r}
#Code here
```



## Wilcoxon tests
The wilcox.test command does both non-parametric Wilcoxon tests (the version of a t-test suitable for data that isn't normally distributed). Here's the rank sum example using lecture data. Think about what the null hypothesis is here. What does a significant p-value tell you?

```{r}
el.nino<-c(0, 5,1,3, 4, 1, 2, 2, 0, 2, 4)
la.nina<-c(4, 4,12, 2, 17, 7, 6, 10, 4, 8, 6)
wilcox.test(el.nino, la.nina)
```

Wilcox.test can also compute *signed rank* tests. This is similar to a matched pairs t-test.

```{r}
data2000<-c(3, 11, 9, 14, 17, 7, 21, 13, 19, 5)
data2010<-c(6, 12, 8, 18, 15, 13, 25, 9, 24, 10)
wilcox.test(data2000, data2010, paired=TRUE)
```


##You try it!
Let's look at a sample dataset of pollen reading for major tree species at several sites in the upper Midwest.

```{r}
pollen_data<-read_csv("data/Midwest_Pollen_Data.csv")

```

Pick two species and determine the normality of their pollen counts. Compare their pollen levels using the appropriate test of difference (t-test/Wilcoxon text). How would you interpret the results?
