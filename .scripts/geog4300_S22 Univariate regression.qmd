...
Create a regression model for temperature and another tree species.
...
Plot/test the residuals. The "residuals" function pulls residuals from the model.
```{r}
...
Graph and map the residuals from the model you created above for a different tree species.
...
##Heteroskedasticity
We can use the Breusch-Pagan test in the lmtest package. The null hypothesis is that the data is uniform, NOT heteroskedastic.
```{r}
library(lmtest)
bptest(model)
```
...
##Outliers
We'll use Cook's Distance to assess outliers This is adapted from: http://r-statistics.co/Outlier-Treatment-With-R.html See also https://onlinecourses.science.psu.edu/stat501/node/340 and http://www.statisticshowto.com/cooks-distance/.
...
##You try it! 
Identify any outliers in your models? Do you have ideas about why the model is doing a poor job with them?
