...
# Similar to other languages like Python, C++, or Java, you use "objects" to do work in R.
# Think of them as boxes. You can store lots of stuff in them.
# Objects can be numeric, string (characters/text), dates, or logical (true/false). 
# They can also be data frames, lists, matrices, or any number of things.
# Both "<-" and "=" can be used to *assign* a value to any object. But <- is more intuitive.

# Let's create an object called "Text" that contains the "Hello, world!" string.
Text<-"Hello, world!"
Text

# Here's another object that's a single number
A<-1
A
...
# # Vectors # # # # 
# When an object is a group of values it is called a vector. 
# Each vector can have only one type of data
...
# You can apply functions to vectors. Here, we add 4 to each number in "A"
A1<-A+4
A1
...
# The `nchar` function gives number of letters in each item in the vector
nchar(C) 

# There's lots of functions in R for mathematical calculation
A<-c(1,1,3,5,6)
...
# # You try it! 
# Create a vector with the ages of three people close to you. Then calculate the mean of those ages.



# # Data frames# # # # 

# Vectors can also grouped together.
# The most common way to do this with a data frame.
# These are tables, just like an Excel spreadsheet.
# Each column in a data frame is like a single vector--a group of values all of the same type.
# You can also mix data types in a data frame--each column is a certain type.
...
# You can apply the same functions shown above when a vector is part of a data frame
mean(test_df$A)
summary(test_df$A)

# # You try it! 
# Use the card decks provided to deal three poker hands (5 cards).
...
# You only have to install a package once, but you need to call it every time you open a project.
...
#  Learn more!
#  The tidyverse is actually comprised of many packages. 
#  Go to https://tidyverse.org/packages. Pick a package and figure out what it does.